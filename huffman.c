...
{
  CAMLparam3 (in, out, insize); // tell the OCaml GC that we have two GC roots here *NOW 3*
  CAMLreturn (Val_int(Huffman_Compress((unsigned char *) Bp_val(in), // unwrap OCaml strings, call, wrap result
			   (unsigned char *) Bp_val(out), Int_val (insize))));
}

/* A stub function which wraps and unwraps OCaml values */
CAMLprim value decode_stub(value in, value out, value insize, value outsize)
{
